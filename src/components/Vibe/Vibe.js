  ...
  return (
    <>
      ...
      {vibes && vibes.length ? (
        ...
        //       </MDBCard>
        <li>
        <h4 onClick={() => setShowInput(!showInput)}>{vibe.title}</h4>
        <input
          ref={inputRef}
          style={{ display: showInput ? 'block' : 'none' }}
          type='text'
          onKeyDown={(e) => {
            if (e.key === 'Enter') {
              ...
              updateVibe(vibe._id, { title })
              setShowInput(false)
            }
          }}
          defaultValue={vibe.title}
        />
        <a href={vibe.url} target='_blank' rel='noreferrer'> {vibe.url}</a>
        <button
          onClick={() => deleteVibe(vibe._id)}
        >
          Delete Me
        </button>
      </li>
